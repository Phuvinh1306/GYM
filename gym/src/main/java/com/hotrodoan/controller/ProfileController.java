...
public class ProfileController {
    ...
    @PutMapping("/update")
    public ResponseEntity<?> updateProfile(HttpServletRequest request, @RequestBody User user) {
        String jwt = jwtTokenFilter.getJwt(request);
        String username = jwtProvider.getUsernameFromToken(jwt);
        User user1 = userService.findByUsername(username).orElseThrow();
        return new ResponseEntity<>(profileService.updateProfile(user, user1.getId()), HttpStatus.OK);
    }
